...
#define sampleFreq	(1.0/0.002)		// sample frequency in Hz
...
void MahonyAHRGetEuler(MahonyAHRSEuler_t * euler)
{
  ...
  euler->yaw = atan2f(2*(q0*q3 + q1*q2), q0*q0 + q1*q1 - q2*q2 - q3*q3) * 180 / M_PI;
  euler->pitch = asinf(gx) * 180 / M_PI; //Pitch seems to be inverted
  euler->roll = atan2f(gy, gz) * 180 / M_PI;
}
